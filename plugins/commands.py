...
def formate_file_name(file_name):
    ...
    for c in chars:
        file_name.replace(c, "")
    file_name = '@x7islandDevs ' + ' '.join(filter(lambda x: not x.startswith('http') and not x.startswith('@') and not x.startswith('www.'), file_name.split()))
    return file_name
...
async def start(client, message):
    ...
    if len(message.command) != 2:
        buttons = [[
            InlineKeyboardButton('🍃 ᴏғғɪᴄɪᴀʟ ᴡᴇʙ', url='https://t.me/x7islandDevs')
            ],[
            InlineKeyboardButton('💝 ᴊᴏɪɴ ᴜs ᴏɴ ɪɴsᴛᴀ', url='https://instagram.com/7stoical'),
            InlineKeyboardButton('🤖 ᴜᴘᴅᴀᴛᴇ ᴄʜᴀɴɴᴇʟ', url='https://t.me/x7islandDevs')
            ],[
